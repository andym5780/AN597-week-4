...
#where t is equal to the combined mean of the heights
t <- (99*mean(set)+1*m)/100
t
#where g is equal to the combined sd of height
g <- sqrt(((99-1)*var(set))+((1-1)*v)/(100-2))
g
```
I keep gettingt this error on every single time I try it so this is what should work or at least that what wverywhere I looked said but I just can't quite get it, I even tried to use the na.rm = TRUE but couldnt get that to work so I dont know what to do. This then makes it difficult to answer the rest of the questions that are in problem 6
```{r}
x <- samp$weight
m <- mean(x)
v <- var(x)
...
#where t is equal to the combined mean of the weight
t <- (99*mean(set)+1*m)/100
t
#where g is equal to the combined sd of weight
g <- sqrt(((99-1)*var(set))+((1-1)*v)/(100-2))
g
```
```{r}
...
#where t is equal to the combined mean of the age
t <- (99*mean(set)+1*m)/100
t
#where g is equal to the combined sd of height
g <- sqrt(((99-1)*var(set))+((1-1)*v)/(100-2))
...
#where t is equal to the combined mean of the zombies killed
t <- (99*mean(set)+1*m)/100
t
#where g is equal to the combined sd of height
g <- sqrt(((99-1)*var(set))+((1-1)*v)/(100-2))
...
#where t is equal to the combined mean of the years of education
t <- (99*mean(set)+1*m)/100
t
#where g is equal to the combined sd of height
g <- sqrt(((99-1)*var(set))+((1-1)*v)/(100-2))
